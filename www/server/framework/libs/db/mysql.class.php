...
class mysql{
	...
	function connect($config){
		extract($config);//将关联数组导入到当前的符号表中，键名作为变量名，值作为变量值
		if(!($conn = new mysqli($dbhost,$dbuser,$dbpsw,$dbname))){//mysql_connect连接数据库函数
			$this->err($conn -> connect_error) ;
		}
		$conn->query("set names ".$dbcharset);//使用mysql_query 设置编码  格式：mysql_query("set names utf8")
	}
	/**
	 * 执行sql语句
	 *
	 ...
	 */
	function query($sql){
		if(!($query = mysqli_query($conn,$sql))){//使用mysql_query函数执行sql语句
			$this->err($sql."<br />".$conn->connectt_error);//mysql_error 报错
		}else{
			return $query;
		}
	}
	...
	**/
	function findAll($query){
		while($rs=$conn->fetch_array($query, MYSQL_ASSOC)){//mysql_fetch_array函数把资源转换为数组，一次转换出一行出来
			$list[]=$rs;
		}
	...
	**/
	function findOne($query){
		$rs = $conn->fetch_array($query, MYSQL_ASSOC);
		return $rs;
	}
	...
	**/
	function findResult($query, $row = 0, $filed = 0){
		$rs = $conn->result($query,  $row, $filed);
		return $rs;
	}
